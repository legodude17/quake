...
class Quake extends Orchestrator {
  constructor(renderer) {
    ...
    this.on("stop", () => {
      this._renderer.end();
      console.log(symbols.success + " BUILD FINISHED after ~" + time(process.hrtime(this.startTime)));
    });
    this.on("err", err => {
      this._renderer.end();
  ...
  add(name, dep, fn) {
    if (!fn && (typeof dep === 'function' || Array.isArray(dep))) {
			fn = dep;
			dep = undefined;
		}
    var func;
  ...
  src(file, enc) {
    return this.name((res, cb) => fs.readFile(this.resolve(file), enc || 'utf-8', (err, c) => {Array.isArray(res) ? res.push(c) : res = c; cb(err, res)}), "Load file " + file);
  }

  dest(file, enc) {
    return this.name((contents, cb) => fs.writeFile(this.resolve(file), contents, cb), "Save file " + file);
  }
