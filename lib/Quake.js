...
class Quake extends Orchestrator {
  ...
  src(file, enc) {
    if (globby.hasMagic(file)) {
      return this.name(res => fileUtils.readAll(file, enc).then(reses => Array.isArray(res) ? res.concat(reses) : reses), "Load files: " + file);
    }
  ...
  dest(file) {
    if(globby.hasMagic(file)) {
      return this.name(res => fileUtils.writeAll(file, res), "Save files: " + file);
    }
  ...
  create(file, override, toClean) {
    if (globby.hasMagic(file)) {
      return fileUtils.createAll(file, override);
    }
    return this.name((contents, cb) => {
      fs.access(file, err => err && !override ? fs.writeFile(file, "", cb) : cb(null, false))
    }, "Make file " + file);
  }
  ...
  sync(fn) {
    return this.name((arg, cb) => cb(null, fn(arg)), fn.displayName || fn.name);
